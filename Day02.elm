        ...
        |> List.map
            (\row ->
                case ( List.maximum row, List.minimum row ) of
                    ...
                    _ ->
                        Debug.crash "Empty rows not possible!"
            )
        |> List.sum
...
rowDivision numbers =
    numbers
        |> List.filterMap
            (\num ->
                numbers
                    |> List.filter (\d -> not (d == num) && (num % d == 0))
                    |> List.head
                    |> Maybe.map (\div -> num // div)
            )
        |> (\list ->
                case list of
                    d :: [] ->
                    ...
                    _ ->
                        Debug.crash "There's more or less than one divisible pair. I was told this would not happen!"
           )


toNumberRows : String -> List (List Int)
toNumberRows =
    String.lines >> List.map (String.words >> List.filterMap (String.toInt >> Result.toMaybe))
